...
backend_redis_create() {
  ...
  docker run --name mysql-${instancia_add} -e MYSQL_ROOT_PASSWORD=${mysql_root_password} -e MYSQL_DATABASE=${instancia_add} -e MYSQL_USER=${instancia_add} -e MYSQL_PASSWORD=${mysql_root_password} --restart always -p ${mysql_port:3306}:3306 -d mariadb:latest --character-set-server=utf8mb4 --collation-server=utf8mb4_bin
